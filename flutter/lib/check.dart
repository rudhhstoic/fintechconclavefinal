...
class _BudgetPageState extends State<BudgetPage> {
  ...
  Future<void> setBudget(String category, String limit) async {
    final response = await http.post(
      Uri.parse('http://your_server_ip:5004/set_budget'),
  ...
  void showSetBudgetDialog(String? selectedCategory, [String? limit]) {
    TextEditingController limitController =
        TextEditingController(text: limit ?? '');
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text(selectedCategory == null
            ? "Set New Budget"
            : "Update Budget for $selectedCategory"),
        content: Column(
        ...
        actions: [
          TextButton(
            onPressed: () {
              if (selected != null && limitController.text.isNotEmpty) {
                setBudget(selected!, limitController.text);
                Navigator.of(context).pop();
              } else {
                print("Please select a category and enter a limit.");
              }
            },
            child: Text("Set"),
          ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () {
                        showSetBudgetDialog(
                            budget['category'], budget['limit'].toString());
                      },
                    ),
                  ),
                );
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          showSetBudgetDialog("null"); // Customize as needed
        },
